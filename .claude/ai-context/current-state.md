...
- **Khoá khu soạn nội dung**: đặt `IMPORT_PASSWORD` thì `/import*` chuyển hướng sang `/import/khoa` để nhập mật khẩu, và mọi API ghi (`create-lesson`, `update-lesson`, `chapters` POST, `upload-image`, `upload-audio`, `lesson-audio`, `ocr-exam`) tự kiểm tra — proxy không chạy cho `/api/*`. Bỏ trống biến thì mở như cũ
