...
import { QuizResult, getLessonMeta } from "@/lib/quizData";
...
export default function ResultPage() {
  ...
  const { questions, answers, lessonId } = result;
        ...
        <div className="max-w-6xl mx-auto px-4 pt-4 pb-6">
          <nav className="flex flex-wrap items-center gap-1 text-xs mb-5">
            {[
              ...
              { label: "Toán", href: "#" },
              { label: lesson.title, href: `/quiz?lessonId=${lessonId}` },
            ].map(({ label, href }) => (
          ...
          <p className="text-center text-sm text-gray-400">{lesson.title}</p>
        </div>
      </div>
