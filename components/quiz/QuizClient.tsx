...
import { getQuestions, getLessonMeta, formatTime } from "@/lib/quizData";
...
export default function QuizClient() {
  ...
  const [{ questions, lesson }] = useState(() => ({
    questions: loadExamQuestions(lessonId, () => getQuestions(lessonId)),
    lesson: loadExamMeta(lessonId, () => getLessonMeta(lessonId)),
  }));
  ...
  const submit = (finalAnswers: (string | null)[]) => {
    sessionStorage.setItem(
      "quizResult",
      JSON.stringify({ questions, answers: finalAnswers, lessonId })
    );
    router.push("/result");
  };
